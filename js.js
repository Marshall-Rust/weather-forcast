...
// UV index by latitude and longitude coordinates

// Object for Weather conditions city
...
var loadCity = function () {
  ...
  if (dataStore) {
    ...
    for (var i = 0; i < dataStore.length; i++) {
      ...
      liElement.innerHTML =
        "<button type='button' class='list-group-item list-group-item-action' attr='" +
        dataStore[i] +
        "'>" +
        dataStore[i] +
        "</button>";
...
// clean everything is inside the container
...
var weatherHTML = function (city, uv) {
  ...
  ctn3.innerHTML =
    "<p>Temperature: " +
    weatherCondition[0].temp +
    " °F / " +
    converTemp(weatherCondition[0].temp) +
    " °C</p>" +
    "<p>Humidity: " +
    weatherCondition[0].humidity +
    "% </p>" +
    "<p>Wind Speed: " +
    weatherCondition[0].speed +
    " MPH / " +
    convertWSpeed(weatherCondition[0].speed) +
    " KPH </p>" +
    "<p>UV index: <span class='text-white " +
    findUV(uv) +
    "'>" +
    uv +
    "</span></p>";
...
var callApiFetch = function (city) {
  ...
  if (location.protocol === "http:") {
    url =
      "http://api.openweathermap.org/data/2.5/forecast?appid=b262298fbe39ad30d243f31f6e1297bc&units=imperial&q=" +
      city;
  } else {
    url =
      "https://api.openweathermap.org/data/2.5/forecast?appid=b262298fbe39ad30d243f31f6e1297bc&units=imperial&q=" +
      city;
  }
  ...
  fetch(url)
    ...
    .then(function (weatherResponse) {
      if (weatherResponse.cod != "200") {
        displayAlertMessage(
          "Unable to find " + city + " in OpenWeathermap.org"
        );
      ...
      if (location.protocol === "http:") {
        url1 =
          "http://api.openweathermap.org/data/2.5/uvi?appid=b262298fbe39ad30d243f31f6e1297bc&lat=" +
          weatherCondition[0].lat +
          "&lon=" +
          weatherCondition[0].lon;
      } else {
        url1 =
          "https://api.openweathermap.org/data/2.5/uvi?appid=b262298fbe39ad30d243f31f6e1297bc&lat=" +
          weatherCondition[0].lat +
          "&lon=" +
          weatherCondition[0].lon;
      }
      ...
      fetch(url1)
        ...
        .then(function (uvResponse) {
          if (!uvResponse) {
            ...
            displayAlertMessage(
              "OpenWeathermap.org could not find anything for latitude and Longitude"
            );
